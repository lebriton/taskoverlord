...
#[derive(Debug, Serialize)]
pub struct GetGroupedTasksResponse {
    group_count: usize,
    task_count: usize,
    data: Vec<TaskGroup>,
}
pub fn get_grouped_tasks() -> Result<GetGroupedTasksResponse> {
    let tasks = get_tasks()?;
    let task_count = tasks.len();

    let tmp = tasks
        .into_iter()
        .into_group_map_by(|task| serde_json::to_string(&task.due).unwrap_or_default());
    let data: Vec<TaskGroup> = tmp
        .into_iter()
        .map(|(grouper, tasks)| TaskGroup { name: grouper, tasks })
        .collect();

    Ok(GetGroupedTasksResponse {
